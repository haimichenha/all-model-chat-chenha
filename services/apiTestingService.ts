...
class ApiTestingService {
  ...
  const originalSettings = (geminiServiceInstance as any)['currentAppSettings'] as AppSettings | null;
      
      // Temporarily update settings for testing
  const base: AppSettings = originalSettings ? { ...originalSettings } : { ...DEFAULT_APP_SETTINGS };
  const testSettings: AppSettings = { ...base, modelId, apiProxyUrl: endpoint || null };
      
  geminiServiceInstance.updateSettings(testSettings);
      
      try {
        // Attempt to send a test message (non-stream)
        await new Promise<void>((resolve, reject) => {
          geminiServiceInstance.sendMessageNonStream(
            ...
            (parts) => {
              ...
              if (text) resolve(); else reject(new Error('收到空响应'));
            }
          );
        });
        return { success: true };
  ...
  const restore = originalSettings ? originalSettings : DEFAULT_APP_SETTINGS;
  geminiServiceInstance.updateSettings(restore);
      }
