...
export const useFileHandling = ({
    ...
    const handleProcessAndAddFiles = useCallback(async (files: FileList | File[]) => {
        ...
        // [核心修复] 使用一个大的 if/else 块来处理 fileMetadata 是否存在
        if (fileMetadata) {
            // --- 所有使用 fileMetadata 的代码都必须在这个 if 块内部 ---
            
            logService.info(`Successfully fetched metadata for file ID ${fileApiId}`, { metadata: fileMetadata });
            
            // [核心修复] 即使在这里，也要为 mimeType 提供后备值
            const safeMimeType = fileMetadata.mimeType || 'application/octet-stream';

            if (!ALL_SUPPORTED_MIME_TYPES.includes(safeMimeType)) {
                logService.warn(`Unsupported file type for file ID ${fileApiId}`, { type: safeMimeType });
                setSelectedFiles(prev => prev.map(f => f.id === tempId ? { 
                    ...f, 
                    name: fileMetadata.displayName || fileApiId, 
                    type: safeMimeType, // 使用安全值
                    size: Number(fileMetadata.sizeBytes) || 0, 
                    isProcessing: false, 
                    error: `Unsupported file type: ${safeMimeType}`, 
                    uploadState: 'failed' 
                } : f));
                return;
            }
        ...
        const newFile: UploadedFile = { 
            ...
            isProcessing: false, 
            progress: 100, 
            uploadState: fileMetadata.state === 'ACTIVE' ? 'active' : 'failed', 
