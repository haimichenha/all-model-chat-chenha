...
import { SquarePen, Trash2, X, Search, PanelLeftClose, MoreHorizontal, Pin, PinOff, Download, Upload, FolderPlus, ChevronDown, FileText } from 'lucide-react';
...
export const HistorySidebar: React.FC<HistorySidebarProps> = ({
  ...
  onExportChatHistory, onImportChatHistory, themeColors, t, language, themeId
}) => {
  ...
  return (
    ...
    >
      ...
      {/* 底部按钮区域 */}
      <div className="mt-auto p-3 space-y-2 border-t border-[var(--theme-border-secondary)]">
        {/* 聊天记录导入/导出按钮组（文本格式） */}
        <div className="flex gap-2">
          <button 
            onClick={onExportChatHistory}
            className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-xs text-[var(--theme-text-secondary)] bg-transparent border border-[var(--theme-border-secondary)] rounded-md hover:bg-[var(--theme-bg-tertiary)] hover:text-[var(--theme-text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--theme-border-focus)] transition-colors"
            aria-label="导出聊天记录为文本"
            title="导出所有聊天记录为文本文件（易读格式）"
          >
            <FileText size={14} />
            <span>导出聊天</span>
          </button>
          <button 
            ...
            className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-xs text-[var(--theme-text-secondary)] bg-transparent border border-[var(--theme-border-secondary)] rounded-md hover:bg-[var(--theme-bg-tertiary)] hover:text-[var(--theme-text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--theme-border-focus)] transition-colors"
            aria-label="从文本导入聊天记录"
            title="从文本文件导入聊天记录（易读格式）"
          >
            <FileText size={14} />
            <span>导入聊天</span>
          </button>
