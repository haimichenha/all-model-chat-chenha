...
interface SettingsModalProps {
  ...
  activeApiConfig?: string | null;
  handleActiveApiConfigChange?: (configId: string | null) => void;
  refreshTrigger?: number;
  // 添加标签页状态管理
  defaultActiveTab?: SettingsTab;
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  onInstallPwa, isInstallable, t, onImportSettings, onExportSettings, onPersistentStoreImportSuccess,
  useCustomApiConfig, setUseCustomApiConfig, activeApiConfig, handleActiveApiConfigChange, refreshTrigger,
  defaultActiveTab = 'general', onTabChange
}) => {
  const [settings, setSettings] = useState(currentSettings);
  const [activeTab, setActiveTab] = useState<SettingsTab>(defaultActiveTab);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // 只有在初次打开时设置默认标签页，避免API操作后跳转
      if (!document.getElementById('settings-modal-opened')) {
        setActiveTab(defaultActiveTab);
        // 设置一个标志，表示模态框已经打开过
        const flag = document.createElement('div');
        flag.id = 'settings-modal-opened';
      ...
      // 关闭模态框时移除标志
      const flag = document.getElementById('settings-modal-opened');
      if (flag) {
        document.body.removeChild(flag);
      }
    }
  }, [isOpen, currentSettings, defaultActiveTab]);
  ...
  const updateSetting = <K extends keyof AppSettings>(key: K, value: AppSettings[K]) => {
    if (key === 'activeSystemPromptId' && value) {
      // 当选择新的系统提示ID时，自动更新系统指令
      const promptId = value as string;
      const selectedPrompt = settings.systemPrompts?.find(p => p.id === promptId);
      if (selectedPrompt) {
        setSettings(prev => ({ 
          ...prev, 
          [key]: value,
            ...
            <div className="p-3 sm:p-5 tab-content-enter-active">
              {activeTab === 'general' && (
                <div className="space-y-4">
                  ...
                  <DataManagementSection
                    ...
                    onPersistentStoreImportSuccess={onPersistentStoreImportSuccess}
                    t={t}
                  />
                </div>
              )}
              ...
              {activeTab === 'model' && (
                <ChatBehaviorSection
                  ...
                  setSystemPrompts={(val) => updateSetting('systemPrompts', val)}
                  activeSystemPromptId={settings.activeSystemPromptId}
                  setActiveSystemPromptId={(val) => updateSetting('activeSystemPromptId', val)}
                  t={t}
                />
              )}
              {activeTab === 'about' && (
