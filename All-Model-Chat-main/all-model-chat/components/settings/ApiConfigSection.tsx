...
export const ApiConfigSection: React.FC<ApiConfigSectionProps> = ({
  ...
  useEffect(() => {
    ...
    setTempProxyUrl(activeConfig.apiProxyUrl || ''); // 确保 tempProxyUrl 始终是字符串
  ...
  const handleSaveConfig = () => {
    ...
    try {
      if (activeApiConfigId && !isAddingNew) {
        ...
        // 创建新配置
        const configName = newConfigName.trim() || `API配置 ${apiConfigs.length + 1}`;
        ...
        console.log('Created new config:', updatedConfig);
      ...
      // 重新加载配置列表以反映最新状态
      const refreshedConfigs = persistentStoreService.getApiConfigs();
      setApiConfigs(refreshedConfigs);
      ...
      alert('保存API配置失败: ' + (error as Error).message);
    }
  };
  ...
  const handleSelectConfig = (configId: string) => {
    ...
    if (config) {
      ...
      setTempProxyUrl(config.apiProxyUrl || '');
  ...
  const handleDeleteConfig = (configId: string) => {
    ...
    if (configId === activeApiConfigId) {
      ...
      if (newCurrentConfig) {
        ...
        setTempProxyUrl(newCurrentConfig.apiProxyUrl || '');
                ...
                >
                  ...
                  >
                    ...
                    <div className="text-xs text-[var(--theme-text-tertiary)]">
                      {config.apiProxyUrl ? `代理: ${config.apiProxyUrl}` : '无代理'}
                      {config.isDefault && ' • 默认'}
                    </div>
                  </div>
                  {useCustomApiConfig && (
