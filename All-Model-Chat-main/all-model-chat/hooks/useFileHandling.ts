...
export const useFileHandling = ({
    ...
    const handleProcessAndAddFiles = useCallback(async (files: FileList | File[]) => {
        ...
        for (const file of filesArray) {
            ...
            // 检查是否为文本文件且文件较大
            if (SUPPORTED_TEXT_MIME_TYPES.includes(effectiveMimeType) && file.size > 10000) { // 10KB以上
                try {
                    const content = await file.text();
                    if (content.length > 3000) { // 3000字符以上
                        const shouldAnalyze = confirm(
                            `检测到较大的文本文件"${file.name}"（${Math.round(file.size / 1024)}KB）。\n\n` +
                            `您希望如何处理这个文件？\n\n` +
                            `• 点击"确定" - 进行智能分段分析（推荐）\n` +
                            `• 点击"取消" - 作为普通文件上传\n\n` +
                        ...
                        if (shouldAnalyze) {
                            ...
                            try {
                                // 导入智能分析服务
                                const { intelligentTextAnalysisService } = await import('../services/intelligentTextAnalysisService');
                                const { createProgressDialog } = await import('../hooks/textProcessingUtils');
                                ...
                                if (analysisResult) {
                                    // 发出一个事件，让App组件接收并处理分析结果
                                    const event = new CustomEvent('intelligentAnalysisCompleted', { 
                                        detail: { result: analysisResult, fileName: file.name } 
                                    });
                                ...
                                alert('智能分析失败: ' + (error instanceof Error ? error.message : String(error)));
                                // 如果分析失败，继续常规文件处理
                            }
                        }
                    }
        ...
        const uploadPromises = filesArray.map(async (file) => {
            ...
            // 决定是否需要上传
            const shouldUploadFile = (!isLocalTextFile && !SUPPORTED_IMAGE_MIME_TYPES.includes(effectiveMimeType)) || appSettings.useFilesApiForImages;
