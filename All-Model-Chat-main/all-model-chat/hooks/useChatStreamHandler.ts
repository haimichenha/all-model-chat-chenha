    ...
    const getStreamHandlers = useCallback((
        ...
        const streamOnPart = (part: Part) => {
            ...
            updateAndPersistSessions(prev => prev.map(s => {
                ...
                // [核心修正] 使用generationId精确找到占位消息，而不是依赖最后一条消息
                const targetMessageIndex = messages.findIndex(m => m.id === generationId);
                
                if (isFirstContentPart && targetMessageIndex !== -1) {
                    ...
                    if (thinkingTime !== null) {
                        messages[targetMessageIndex] = { ...messages[targetMessageIndex], thinkingTimeMs: thinkingTime };
                    }
                }
                ...
                if (anyPart.text) {
                    // [核心修正] 严格遵循"先占位，后填充"模式
                    if (targetMessageIndex !== -1 && !isToolMessage(messages[targetMessageIndex])) {
                        // 找到了占位消息，直接追加内容
                        messages[targetMessageIndex] = {
                            ...messages[targetMessageIndex],
                            content: messages[targetMessageIndex].content + anyPart.text
                        };
                    } else {
                        // 如果没找到占位消息或是工具消息，创建新消息
                        if (targetMessageIndex === -1) {
                            logService.warn('Target message not found for generationId:', generationId);
                        }
        ...
        const onThoughtChunk = (thoughtChunk: string) => {
            updateAndPersistSessions(prev => prev.map(s => {
                ...
                // [核心修正] 使用generationId精确找到目标消息
                const targetMessageIndex = messages.findIndex(m => m.id === generationId);
                if (targetMessageIndex !== -1 && messages[targetMessageIndex].isLoading) {
                    messages[targetMessageIndex] = {
                        ...messages[targetMessageIndex],
                        thoughts: (messages[targetMessageIndex].thoughts || '') + thoughtChunk
                    };
                }
                return { ...s, messages };
            }));
